...
Wuction은 **Spring Framework를 사용하지 않고**,
Java **Servlet/JSP 기반으로 MVC 구조를 직접 구현한 웹 경매(Auction) 서비스**입니다.
프레임워크에 의존하지 않고 웹 애플리케이션의 요청 흐름, 서버 구조, DB 연동 원리를 깊이 이해하는 것을 목표로 개발하였습니다.

회원 관리, 상품 등록, 입찰 기능까지의 경매 핵심 흐름을 직접 구현하였으며,
AWS EC2와 RDS 환경에 직접 배포하여 로컬 환경을 넘어 실제 서버 환경에서의 동작 흐름과 네트워크 설정, 배포 과정을 경험하였습니다.

본 프로젝트에서는 낙찰, 결제, 배송과 같은 후속 프로세스는 범위에서 제외하고,
웹 애플리케이션의 기본 구조와 서버-DB 연동, 요청 처리 흐름에 집중하였습니다.
이를 통해 복잡한 비즈니스 도메인보다는 Java 웹 애플리케이션의 기초 원리와 아키텍처 이해를 우선 목표로 한 학습 중심 프로젝트로 설계하였습니다.

본 프로젝트는 상용 목적이 아닌,
Java 웹 기본 원리 학습 및 백엔드 기초 역량 강화를 위한 포트폴리오 프로젝트입니다.
